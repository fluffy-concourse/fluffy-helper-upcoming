...
class ServerOwnerAndBotDev__Config__SetChannelId(commands.Cog):
    ...
    async def set_channel_id(self, ctx: Context, channel_name: Channels = None, use_channel: discord.TextChannel = None):
        ...
        if ctx.interaction:
            if channel_name != None and use_channel != None:
                try:
                    SemiData.server_config_conn.execute(f'UPDATE channel_ids SET channel_id = ? WHERE channel_name = ? AND server_id = ?', (use_channel.id, channel_name.value, ctx.guild.id))
